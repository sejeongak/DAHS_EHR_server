...
def configure_optimizers(model, args, n_steps):
    ...
    pretrained_params = [p for n, p in model.named_parameters() if "classifier" not in n and p.requires_grad]
    ...
    {'params': pretrained_params, 'lr': base_lr, 'weight_decay': 0.05},
...
def main(args):
    ...
    train_dataset = EHR_Longformer_Dataset(Path("./datasets"), "train", tokenizer, itemid2idx, unit2idx, vocab_size=args.vocab_size, use_itemid=True, mode=args.mode, index=args.index, mask_mode=args.mask_mode, task=args.task)
    valid_dataset = EHR_Longformer_Dataset(Path("./datasets"), "valid", tokenizer, itemid2idx, unit2idx, vocab_size=args.vocab_size, use_itemid=True, mode=args.mode, index=args.index, mask_mode=args.mask_mode, task=args.task)
