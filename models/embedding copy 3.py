...
class EHREmbedding(nn.Module):
    def __init__(self, 
        ...
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.weight_concept = nn.Parameter(torch.ones(1))
        self.weight_time = nn.Parameter(torch.ones(1))
        self.weight_position = nn.Parameter(torch.ones(1))
        self.weight_value = nn.Parameter(torch.ones(1))
        self.weight_unit = nn.Parameter(torch.ones(1))
        self.weight_ordername = nn.Parameter(torch.ones(1))
        self.weight_orderdescription = nn.Parameter(torch.ones(1))
    ...
    def forward(self, 
        ...
        embeddings = (
            self.weight_concept * concept_embed +
            self.weight_time * time_embed +
            self.weight_position * positional_embed +
            self.weight_value * value_embed +
            self.weight_unit * unit_embed +
            self.weight_ordername * ordername_embed +
            self.weight_orderdescription * orderdescription_embed
        )
