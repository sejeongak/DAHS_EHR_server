...
class EnhancedValueEmbedding(nn.Module):
    """Improved Embedding layer for value features"""
    ...
    def __init__(self, hidden_size, dropout_rate=0.1):
        ...
        self.value_embedding = nn.Sequential(
            ...
            nn.GELU(),  # Use GELU for smoother gradients
            nn.Dropout(dropout_rate),  # Add dropout for regularization
            nn.Linear(hidden_size, hidden_size),
            nn.BatchNorm1d(hidden_size),  # Another Batch Normalization
            nn.GELU(),
            nn.Dropout(dropout_rate)  # Another dropout
        )
        
    def forward(self, x):
        x = x.unsqueeze(-1)  # Ensure input is 2D (batch_size, 1)
        x = self.value_embedding(x)
        return x

# class EnhancedValueEmbedding(nn.Module):
#     """ Enhanced Value Embedding with Binning and Feature Interaction """
    
#     def __init__(self, hidden_size, n_bins=256):
#         super().__init__()
#         self.n_bins = n_bins
#         self.hidden_size = hidden_size
        
#         # Magnitude embedding for binned values
#         self.magnitude_embedding = nn.Embedding(n_bins, hidden_size)
        
#         # Linear transformation for additional learning flexibility
#         self.projection = nn.Linear(hidden_size, hidden_size)
#         self.LayerNorm = nn.LayerNorm(hidden_size)
#         self.dropout = nn.Dropout(0.1)

#     def forward(self, value_ids):
#         """
#         Args:
#             value_ids (torch.Tensor): Pre-binned value IDs (batch_size, seq_len)
#         Returns:
#             torch.Tensor: Value embeddings (batch_size, seq_len, hidden_size)
#         """
#         # Embedding binned value
#         magnitude_embeds = self.magnitude_embedding(value_ids)
        
#         # Apply linear projection and normalization
#         value_embeds = self.projection(magnitude_embeds)
#         value_embeds = self.LayerNorm(value_embeds)
#         value_embeds = self.dropout(value_embeds)
...
class PositionalEmbedding(nn.Module):
    ...
    def _init_posi_embedding(self, max_len, hidden_size):
        ...
        lookup_table = torch.zeros(max_len, hidden_size)
...
class EHREmbedding(nn.Module):
    def __init__(self, config: LongformerConfig, itemid_size, unit_size, max_age, max_len, continuous_size, gender_size, task_size, padding_idx=0, use_itemid=True, inputs_embeds=None):
        ...
        self.value_embedding = EnhancedValueEmbedding(self.hidden_size)
