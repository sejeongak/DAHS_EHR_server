...
def train(
    ...
    for epoch in tqdm(range(start_epoch, epochs), desc='Epochs', total=epochs-start_epoch, smoothing=0.1):
                ...
                with accelerator.autocast():
                    ...
                    if outputs['binary_logits'] is not None:
                        ...
                        if args.task_mode == "binary" and args.task_idx is not None:
                            i = args.task_idx
                            loss_i = binary_criterion(logits.squeeze(1), binary_labels[:, i])
                            binary_losses.append(loss_i)
                            binary_task_losses[i].append(loss_i.item())
                            all_preds.append(torch.sigmoid(logits).detach().cpu())
                            all_labels.append(binary_labels[:, i:i+1].detach().cpu())
                        else:  
                            for i in range(args.num_binary_tasks):
                                loss_i = binary_criterion(logits[:, i], binary_labels[:, i])
                                binary_losses.append(loss_i)
                                binary_task_losses[i].append(loss_i.item())
                            all_preds.append(torch.sigmoid(logits).detach().cpu())
                    ...
                    if outputs['sofa_logits'] is not None:
                        ...
                        if torch.isnan(sofa_logits).any():
                            print("[Error] NaN detected in sofa_logits")
                        ...
                        for i in range(args.num_sofa_tasks):
                            pred_i = sofa_logits[:, i, :]  # [B, C]
                            label_i = sofa_labels[:, i]    # [B]
                    ...
                    if outputs['phenotype_logits'] is not None:
                        ...
                        if torch.isnan(phenotype_logits).any():
                            print("[Error] NaN detected in phenotype_logits")
                    ...
                    if loss_weighter is not None:
                        loss = loss_weighter(binary_loss, multiclass_loss, phenotype_loss)
                    ...
                    if outputs['binary_logits'] is not None:
                        ...
                        if args.task_mode == 'binary' and args.task_idx is not None:
                            ...
                            taskwise_metrics = calculate_binary_metrics(
                                torch.cat(all_preds).numpy(), torch.cat(all_labels).numpy(), num_tasks=args.num_binary_tasks)
        ...
        if args.task_mode == 'binary':
            ...
            if args.task_idx is not None:
                ...
                taskwise_metrics = calculate_binary_metrics(binary_preds, binary_labels, num_tasks=args.num_binary_tasks)
        elif args.task_mode == 'multiclass':
            sofa_preds = [torch.cat(p).numpy() for p in all_sofa_preds]
            sofa_labels = [torch.cat(l).numpy() for l in all_sofa_labels]
            sofa_metrics = calculate_multiclass_metrics(sofa_preds, sofa_labels, num_tasks=args.num_sofa_tasks, num_classes=args.num_multiclass_labels)
        elif args.task_mode == 'multilabel':
        ...
        # for i in range(args.num_sofa_tasks):
        #     preds = torch.cat(all_sofa_preds[i], dim=0)  # [N, C]
        #     pred_classes = torch.argmax(preds, dim=1)
        #     print(f"[EPOCH {epoch}] SOFA Task {i} Predicted Class Distribution: {np.unique(pred_classes.numpy(), return_counts=True)}")
        # for i in range(args.num_sofa_tasks):
        #     labels = torch.cat(all_sofa_labels[i], dim=0)
        #     print(f"[EPOCH {epoch}] SOFA Task {i} Label Distribution: {np.unique(labels.numpy(), return_counts=True)}")
        ...
        if accelerator.is_local_main_process:
            ...
            if args.task_mode == 'binary':
                if args.task_idx is not None:
                    metrics = taskwise_metrics[0]
                    log_data.update({
                        f"Train {multitask_labels[args.task_idx]} AUROC": metrics['auroc'],
                        f"Train {multitask_labels[args.task_idx]} AUPRC": metrics['auprc'],
                        f"Train {multitask_labels[args.task_idx]} Loss": np.mean(binary_task_losses[args.task_idx]),
                    })
                else:
                    for i, metrics in enumerate(taskwise_metrics):
                        log_data.update({
                            ...
                            f"Train {multitask_labels[i]} Loss": np.mean(binary_task_losses[i]),
                        })
            elif args.task_mode == 'multiclass':
                for i, metrics in enumerate(sofa_metrics):
                    log_data.update({
        ...
        valid_loss, valid_multitask_metrics, valid_multiclass_metrics, valid_phenotype_metrics = validation(
            ...
            multitask_labels, multiclass_labels, multilabel_labels, binary_criterion, multiclass_criterion, epoch + 1, args)
        ...
        if args.task_mode == 'binary':
            ...
            mean_auroc = np.nanmean([m['macro_auroc'] for m in valid_multiclass_metrics])
            mean_auprc = np.nanmean([m['macro_auprc'] for m in valid_multiclass_metrics])
        elif args.task_mode == 'multilabel':
            mean_auroc = valid_phenotype_metrics['macro_auroc']
            mean_auprc = valid_phenotype_metrics['macro_auprc']
        elif args.task_mode == 'multitask':
    ...
    test_loss, test_multitask_metrics, test_multiclass_metrics, test_phenotype_metrics = test(
            device, model, test_loader, accelerator,
            multitask_labels, multiclass_labels, multilabel_labels, binary_criterion, multiclass_criterion, args)
...
def validation(
    ...
    binary_task_losses = [[] for _ in range(args.num_binary_tasks)]
    ...
    if args.task_mode == 'binary':
        ...
        if args.task_idx is not None:
            taskwise_metrics = calculate_binary_metrics(all_preds, all_labels, num_tasks=1)
        else:
            taskwise_metrics = calculate_binary_metrics(all_preds, all_labels, num_tasks=args.num_binary_tasks)
        sofa_metrics = None
        phenotype_metrics = None
    elif args.task_mode == 'multiclass':
    ...
    if accelerator.is_local_main_process:
        ...
        if args.task_mode == 'binary':
            if args.task_idx is not None:
                metrics = taskwise_metrics[0]
                log_data.update({
                    f"Validation {multitask_labels[args.task_idx]} AUROC": metrics['auroc'],
                    f"Validation {multitask_labels[args.task_idx]} AUPRC": metrics['auprc'],
                    f"Validation {multitask_labels[args.task_idx]} Loss": np.mean(binary_task_losses[args.task_idx]),
                })
            else:
                for i, metrics in enumerate(taskwise_metrics):
                    log_data.update({
                        ...
                        f"Validation {multitask_labels[i]} Loss": np.mean(binary_task_losses[i]),
                    })
        elif args.task_mode == 'multiclass':
            for i, metrics in enumerate(sofa_metrics):
                log_data.update({
    ...
    if accelerator.is_local_main_process:
        ...
        if args.task_mode == 'binary':
            if args.task_idx is not None:
                metrics = taskwise_metrics[0]
                log_data.update({
                    f"Test {multitask_labels[args.task_idx]} AUROC": metrics['auroc'],
                    f"Test {multitask_labels[args.task_idx]} AUPRC": metrics['auprc'],
                    f"Test {multitask_labels[args.task_idx]} Loss": np.mean(binary_task_losses[args.task_idx]),
                })
            else:  # multitask or all tasks
                for i, metrics in enumerate(taskwise_metrics):
                    log_data.update({
                        f"Test {multitask_labels[i]} AUROC": metrics['auroc'],
                        f"Test {multitask_labels[i]} AUPRC": metrics['auprc'],
                        f"Test {multitask_labels[i]} Loss": np.mean(binary_task_losses[i]),
                    })
