...
def calculate_multiclass_metrics(preds, labels, num_tasks=5, num_classes=5, average="macro"):
    task_metrics = []
    ...
    for i in range(num_tasks):
        ...
        # one-hot 또는 multilabel 형식으로 변환
        true_onehot = label_binarize(true, classes=np.arange(num_classes))
        ...
        if len(np.unique(true)) < 2:
            ...
            auroc = roc_auc_score(true_onehot, prob, average=average, multi_class="ovr")
            auprc = average_precision_score(true_onehot, prob, average=average)
        ...
        task_metrics.append({
            ...
            "precision": precision_score(true, pred_classes, average=average, zero_division=0),
            "recall": recall_score(true, pred_classes, average=average, zero_division=0),
            "f1_score": f1_score(true, pred_classes, average=average, zero_division=0),
        })
...
def train(
    ...
    for epoch in tqdm(range(start_epoch, epochs), desc='Epochs', total=epochs-start_epoch, smoothing=0.1):
        ...
        all_preds = []
        all_labels = []
        binary_losses = [[] for _ in range(args.num_binary_tasks)]
        
        multiclass_losses = [[] for _ in range(args.num_sofa_tasks)]
        ...
        for step, batch in tqdm(enumerate(data_loader), desc="Steps", total=len(data_loader), leave=False):
            with accelerator.accumulate(model):
                ...
                with accelerator.autocast():
                    ...
                    labels = labels[: -args.num_sofa_tasks].float()
                    
                    sofa_logits = outputs['sofa_logits'].squeeze(1)
                    sofa_labels = labels[-args.num_sofa_tasks:].float()
                    
                    
                    phenotype_logits = outputs['phenotype_logits']
                    ...
                    for i in range(args.num_binary_tasks):
                        binary_loss = criterion(logits[:, i], labels[:, i])
                        # Uncertainties
                        # weighted_loss = binary_loss / (2 * model.module.task_uncertainties[i] ** 2) + torch.log(model.module.task_uncertainties[i])
                        # binary_losses.append(weighted_loss)
                        binary_losses.append(binary_loss)
                        binary_losses[i].append(binary_loss.item())
                    binarytask_loss = sum(binary_losses) 
                    ...
                    for i in range(args.num_sofa_tasks):
                        multiclass_loss = criterion(sofa_logits[:, i], sofa_labels[:, i])
                        # Uncertainties
                        # weighted_loss = multiclass_loss / (2 * model.module.task_uncertainties[i] ** 2) + torch.log(model.module.task_uncertainties[i])
                        # multiclass_losses.append(weighted_loss)
                        multiclass_losses.append(multiclass_loss)
                        multiclass_losses[i].append(multiclass_loss.item())
                    ...
                    loss = binarytask_loss + multiclass_loss + phenotype_loss
                ...
                if step != 0 and step % 100 == 0 and accelerator.is_local_main_process:
                    taskwise_metrics = calculate_taskwise_metrics(probs.detach().cpu().numpy(), labels.detach().cpu().numpy(), num_tasks=args.num_binary_tasks)
        ...
        taskwise_metrics = calculate_taskwise_metrics(all_preds, all_labels, num_tasks=args.num_binary_tasks)
        ...
        if accelerator.is_local_main_process:
            ...
            for i, metrics in enumerate(taskwise_metrics):
                log_data.update({
                    ...
                    f"Train {multitask_labels[i]} Loss": np.mean(binary_losses[i]),
                })
            log_data.update({
