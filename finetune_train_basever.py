        ...
        for step, batch in tqdm(enumerate(data_loader), desc="Steps", total=len(data_loader), leave=False):
            with accelerator.accumulate(model):
                ...
                if step != 0 and step % 100 == 0 and accelerator.is_local_main_process:
                    ...
                    # for i in range(args.num_sofa_tasks):
                    #     sofa_check_preds = torch.cat(all_sofa_preds[i], dim=0)  # shape: [N, C]
                    #     sofa_preds_class = torch.argmax(sofa_check_preds, dim=1)      # shape: [N]
                    #     print(f"[STEP {step}] {multiclass_labels[i]} Predicted Classes: {np.unique(sofa_preds_class.numpy(), return_counts=True)}")
                    #     labels = torch.cat(all_sofa_labels[i], dim=0)
                    #     print(f"[STEP {step}] {multiclass_labels[i]} True Label Distribution: {np.unique(labels.numpy(), return_counts=True)}")
...
def validation(
            ...
            if step % 100 == 0 and accelerator.is_local_main_process:
                ...
                for i, metrics in enumerate(taskwise_metrics):
                    log_data.update({
                        ...
                        f"Step Validation {multitask_labels[i]} Loss": np.mean(binary_task_losses[i]),
                    })
                for i, metrics in enumerate(sofa_metrics):
                    log_data.update({
                        f"Step Validation {multiclass_labels[i]} Macro AUROC": metrics['macro_auroc'],
                        f"Step Validation {multiclass_labels[i]} Macro AUPRC": metrics['macro_auprc'],
                        f"Step Validation {multiclass_labels[i]} Micro AUROC": metrics['micro_auroc'],
                        f"Step Validation {multiclass_labels[i]} Micro AUPRC": metrics['micro_auprc'],
                        f"Step Validation {multiclass_labels[i]} Loss": np.mean(multiclass_task_losses[i]),
                    })
                log_data.update({
    ...
    # all_sofa_preds = [torch.cat(p).numpy() for p in all_sofa_preds]
    # all_sofa_labels = [torch.cat(l).numpy() for l in all_sofa_labels]
    ...
    # # all_sofa_preds = accelerator.gather(all_sofa_preds).cpu().numpy()
    # # all_sofa_labels = accelerator.gather(all_sofa_labels).cpu().numpy()
