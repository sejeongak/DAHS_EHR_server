...
class CAGrad:
    def __init__(self, model, c=0.4, alpha=0.5):
        self.model = model
        self.c = c
        self.alpha = alpha
        self.grad_dims = []
    ...
    def _create_storage(self):
        """Gradient 저장 공간 생성."""
        self.grads = torch.zeros(sum(self.grad_dims), device=next(self.model.parameters()).device)

    def _pack_grad(self, grads, task_num):
        """grad를 grads 벡터에 저장."""
        cnt = 0
        for param in self.model.parameters():
            if param.requires_grad:
                grad = param.grad.detach().clone()
                grads[cnt:cnt + param.data.numel()] = grad.view(-1)
                cnt += param.data.numel()

    def _set_grad(self, new_grads):
        """new_grads를 파라미터에 재설정."""
        cnt = 0
        for param in self.model.parameters():
            if param.requires_grad:
                grad_shape = param.grad.shape
                param.grad = new_grads[cnt:cnt + param.data.numel()].view(grad_shape).clone()
                cnt += param.data.numel()

    def backward(self, losses, accelerator):
        """losses를 이용해서 최적의 합성 gradient 계산."""
        grads = []
        for loss in losses:
            accelerator.backward(loss, retain_graph=True)
            g = torch.zeros(sum(self.grad_dims), device=next(self.model.parameters()).device)
            self._pack_grad(g, 0)
            grads.append(g)
        ...
        GG = grads @ grads.t()
        g0_norm = (GG.mean() + 1e-6).sqrt()

        num_tasks = grads.shape[0]
        alpha = self.alpha
        c = self.c

        w = torch.ones(num_tasks, device=grads.device) / num_tasks
    ...
    for epoch in tqdm(range(start_epoch, epochs), desc='Epochs', total=epochs-start_epoch, smoothing=0.1):
        ...
        for step, batch in tqdm(enumerate(data_loader), desc="Steps", total=len(data_loader), leave=False):
            with accelerator.accumulate(model):
                ...
                cagrad.backward(task_losses + [phenotype_loss], accelerator)
                ...
                total_loss += (sum([l.item() for l in task_losses]) + phenotype_loss.item())
                ...
                if step != 0 and step % 100 == 0 and accelerator.is_local_main_process:
                    ...
                    log_data = {
                        "Step": step+1,
                        # "Step Loss": loss.item(),
                        "Step Loss": sum([l.item() for l in task_losses]) + phenotype_loss.item(),
                    }
