...
def compute_mlm_loss_with_similarity(predictions, labels, similarity_map, idx2itemid):
    
    prediction_score = predictions.argmax(dim=-1).float().to(predictions.device)
    labels = labels.long().to(labels.device)
    # print(prediction_score)
    predictions_item = [idx2itemid[tensor.item()] for tensor in prediction_score]
    label_item = [idx2itemid[tensor.item()] for tensor in labels]
    ...
    # print(base_loss)
    # print("========")
    # print(similarities.view(-1))
    # similarity_loss = base_loss * (1 - similarities.view(-1))
...
def train(
    ...
    for epoch in tqdm(range(start_epoch, epochs), desc="Epochs", total=epochs-start_epoch):
        ...
        for step, batch in tqdm(enumerate(data_loader), desc="Steps", total=len(data_loader)):
            with accelerator.accumulate(model):
                ...
                if args.similarity_mode:
                    mlm_cls_loss = compute_mlm_loss_with_similarity(valid_scores, valid_labels, similarity_map, idx2itemid)
                else:
                ...
                if step != 0 and step % 300 == 0:   
                    if accelerator.is_local_main_process:
                        ...
                        print(f"Epoch {epoch+1} | Step {step} | Train Loss: {train_cls_loss.item():.4f} | MLM Precision: {step_precision:.4f}")
                        # print(f"Epoch {epoch+1} | Step {step} | Train Loss: {train_cls_loss.item():.4f} | MLM Precision: {step_precision:.4f} | Value Regression loss: {mlm_reg_loss.item():.4f}")     
                        wandb.log({
                            "Step": step,
                            "Step Loss": round(train_cls_loss.item(), 4),
        ...
        valid_loss, valid_precision, valid_reg_loss = validation(device, model, val_loader, scaler, accelerator, similarity_map, idx2itemid, args)
...
def validation(
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(data_loader), desc="Validation", total=len(data_loader)):
            ...
            valid_scores = prediction_scores[valid_mask]
            if args.similarity_mode:
                predictions_item = [idx2itemid[tensor.item()] for tensor in all_predictions]
                label_item = [idx2itemid[tensor.item()] for tensor in all_labels]
                mlm_cls_loss = compute_mlm_loss_with_similarity(predictions_item, label_item, similarity_map)
            else:
                mlm_cls_loss = compute_mlm_loss(valid_scores, valid_labels)
