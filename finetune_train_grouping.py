...
def train(
    ...
    for epoch in range(start_epoch, epochs):
        model.train()
        total_loss = 0
        basetask_task_losses = [[] for _ in range(args.num_basetask_tasks)]
        intervention_task_losses = [[] for _ in range(args.num_intervention_tasks)]
        shock_task_losses = []
        sofa_task_losses = [[] for _ in range(args.num_sofa_tasks)]
        phenotype_task_losses = []
                ...
                with accelerator.autocast():
                    ...
                    if args.task_group == "basetask":
                        ...
                        for i in range(args.num_intervention_tasks):
                            loss_i = binary_criterion(logits[:, i], binary_labels[:, i])
                            binary_losses.append(loss_i)
                            intervention_task_losses[i].append(loss_i.item())
                
                        loss = sum(binary_losses)
                        all_preds.append(torch.sigmoid(logits).detach().cpu())
                        all_labels.append(binary_labels.detach().cpu())
                        
                    elif args.task_group == "sofa+shock":
                        logits = outputs['binary_logits'].squeeze(1)
                        binary_labels = labels[:, args.num_binary_tasks-1].unsqueeze(-1).float()
                        binary_loss = binary_criterion(logits, binary_labels)
                        shock_task_losses.append(binary_loss.item())
                        ...
                        for i in range(args.num_sofa_tasks):
                            ...
                            sofa_losses.append(loss_i)
                            sofa_task_losses[i].append(loss_i.item())
                            all_sofa_preds[i].append(torch.softmax(pred_i, dim=1).detach().cpu())
                            all_sofa_labels[i].append(label_i.detach().cpu())
                        sofa_loss = sum(sofa_losses)
                        
                        loss = binary_loss + sofa_loss
                        
                    elif args.task_group == "phenotype":
                        ...
                        phenotype_task_losses.append(phenotype_loss.item())
        ...
        if args.task_group == "basetask":
            ...
            if accelerator.is_local_main_process:
                for i, metrics in enumerate(taskwise_metrics):
                    log_data.update({
                        ...
                        f"Train {multitask_labels[i]} Loss": np.mean(basetask_task_losses[i]),
                    })
                wandb.log(log_data)
            ...
            if accelerator.is_local_main_process:
                for i, metrics in enumerate(taskwise_metrics):
                    log_data.update({
                        ...
                        f"Train {multitask_labels[args.num_basetask_tasks+i]} Loss": np.mean(intervention_task_losses[i]),
                    })
                wandb.log(log_data)
                for k, v in log_data.items():
                    print(f"{k}: {v}")
            ...
            sofa_metrics = calculate_multiclass_metrics(sofa_preds, sofa_labels, num_tasks=args.num_sofa_tasks, num_classes=args.num_multiclass_labels) if sofa_preds is not None else []
            if accelerator.is_local_main_process:
                log_data.update({
                    f"Train {multitask_labels[-1]} AUROC": taskwise_metrics[0]['auroc'],
                    f"Train {multitask_labels[-1]} AUPRC": taskwise_metrics[0]['auprc'],
                    f"Train {multitask_labels[-1]} Loss": np.mean(shock_task_losses),
                })
                for i, metrics in enumerate(sofa_metrics):
                    log_data.update({
                        ...
                        f"Train {multiclass_labels[i]} Loss": np.mean(sofa_task_losses[i]),
                    })
            ...
            if accelerator.is_local_main_process:
                ...
                for i, auroc in enumerate(phenotype_metrics['per_task_auroc']):
                    log_data.update({
                        f"Train {multilabel_labels[i]} AUROC": auroc,
                    })
                for i, auprc in enumerate(phenotype_metrics['per_task_auprc']):
                    log_data.update({
                        f"Train {multilabel_labels[i]} AUPRC": auprc,
                    })
            ...
            mean_auroc = phenotype_metrics['macro_auroc']
            mean_auprc = phenotype_metrics['macro_auprc']
...
def validation(
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(data_loader), desc="Validation", total=len(data_loader)):
            ...
            if args.task_group == "basetask":
                ...
                total_loss += phenotype_loss.item()
    ...
    if args.task_group == "basetask":
        ...
        if accelerator.is_local_main_process:
            log_data.update({
                f"Validation {multitask_labels[-1]} AUROC": taskwise_metrics[0]['auroc'],
                f"Validation {multitask_labels[-1]} AUPRC": taskwise_metrics[0]['auprc'],
            })
            for i, metrics in enumerate(sofa_metrics):
                log_data.update({
        ...
        if accelerator.is_local_main_process:
            ...
            for i, auroc in enumerate(phenotype_metrics['per_task_auroc']):
                log_data.update({
                    f"Validation {multilabel_labels[i]} AUROC": auroc,
                })
            for i, auprc in enumerate(phenotype_metrics['per_task_auprc']):
                log_data.update({
                    f"Validation {multilabel_labels[i]} AUPRC": auprc,
                })
...
def test(
    ...
    if args.task_group == "basetask":
        ...
        if accelerator.is_local_main_process:
            for i, metrics in enumerate(taskwise_metrics):
                log_data.update({
                    f"Test {multitask_labels[args.num_basetask_tasks+i]} AUROC": metrics['auroc'],
                    f"Test {multitask_labels[args.num_basetask_tasks+i]} AUPRC": metrics['auprc'],
                })
            wandb.log(log_data)
            for k, v in log_data.items():
                print(f"{k}: {v}")
        ...
        if accelerator.is_local_main_process:
            log_data.update({
                f"Test {multitask_labels[-1]} AUROC": taskwise_metrics[0]['auroc'],
                f"Test {multitask_labels[-1]} AUPRC": taskwise_metrics[0]['auprc'],
            })
            for i, metrics in enumerate(sofa_metrics):
                log_data.update({
        ...
        if accelerator.is_local_main_process:
            ...
            for i, auroc in enumerate(phenotype_metrics['per_task_auroc']):
                log_data.update({
                    f"Test {multilabel_labels[i]} AUROC": auroc,
                })
            for i, auprc in enumerate(phenotype_metrics['per_task_auprc']):
                log_data.update({
                    f"Test {multilabel_labels[i]} AUPRC": auprc,
                })
